...
class SenderWalletTransaction {
  ...
  factory SenderWalletTransaction.fromMap(Map<String, dynamic> map) {
    ...
    return SenderWalletTransaction(
      ...
      completedAt: rawCompletedDate is Timestamp
          ? rawCompletedDate.toDate()
          : null,
...
class FirebaseSenderWalletRepository implements SenderWalletRepository {
  ...
  Stream<SenderWalletData> watch() async* {
    ...
    await for (final snapshot
      ...
      yield SenderWalletData.fromMap(
        snapshot.data() ?? const {},
        onboardingCompleted:
...
class _SenderWalletViewState extends State<SenderWalletView> {
  ...
  Future<void> _redeemRothCard() async {
    ...
    try {
      await FirebaseFunctions.instance.httpsCallable('redeemGiftCard').call({
        'code': code,
      });
      await _load();
      ...
      if (mounted) {
        _notice(
          context,
          error.message ?? 'This Roth Card could not be redeemed.',
        );
      }
    }
  }
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    [status, _walletTransactionDate(transaction)].join(' · '),
                    style: TextStyle(
                      color: _walletStatusColor(transaction.status),
                      fontSize: 11,
                      fontWeight: FontWeight.w600,
  ...
  Widget build(BuildContext context) {
    ...
    if (option.type == SenderPaymentProfileOptionType.googlePay) {
      return _WalletLink(
        ...
        title: 'Google Pay',
        detail: showDefaultBadge
            ? '✓ Default'
            : 'Use on supported Android devices',

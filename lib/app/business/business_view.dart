...
class _BusinessViewState extends State<BusinessView> {
  ...
  void initState() {
    ...
    _paymentRepository =
        widget.paymentProfileRepository ??
  ...
  Future<void> _load({String? accountId}) async {
    ...
    try {
      ...
      final results = await Future.wait<dynamic>([
        ...
        _repository
            .loadPendingAccessRequests(selected)
            .catchError((_) => <BusinessAccessRequest>[]),
      ]);
      if (!mounted) return;
      setState(() {
        _accounts = accounts;
  ...
  Widget _overview() {
    ...
    final activeDeliveries = data.deliveries
        .where((item) => item.isActive)
        .length;
    final scheduledDeliveries = data.deliveries
        .where((item) => item.isScheduled)
        .length;
    final vanguardDeliveries = data.deliveries
    ...
    final teamActivity = data.account.teamMembers
        .take(3)
        .toList(growable: false);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _deliveries() {
    ...
    final filtered = _workspace!.deliveries
        .where((item) {
    ...
    return Column(
      ...
      children: [
        ...
        SegmentedButton<BusinessDeliverySegment>(
          segments: const [
            ButtonSegment(
              value: BusinessDeliverySegment.active,
              label: Text('Active'),
            ),
            ButtonSegment(
  ...
  Widget _invoices() {
    ...
    final invoices = _workspace!.invoices
        .where(
          (item) =>
              query.isEmpty ||
              '${item.number} ${item.id} ${item.status}'.toLowerCase().contains(
                query,
              ),
        )
        .toList(growable: false);
        ...
        Wrap(
          ...
          children: [
            ...
            _CompactAction(
              ...
              icon: Icons.table_view_rounded,
              onTap: () =>
                  _showMessage('Invoice CSV export is being prepared.'),
            ),
            _CompactAction(
              label: 'VAT Invoices',
            ...
            children: [
              ...
              if (hasCode)
                OutlinedButton.icon(
                  onPressed: _working
                      ? null
                      : () => _ensureCompanyCode(rotate: true),
  ...
  Widget _productRequests({required bool isHealth}) {
    final requests = isHealth
        ? _workspace!.healthRequests
        : _workspace!.giftRequests;
      ...
      children: [
        ...
        if (requests.isEmpty)
          _EmptyState(
            ...
            actionLabel: isHealth
                ? 'Contact Health+ Team'
                : 'Reorder Previous Gift',
  ...
  Widget _analytics() {
    final completed = _workspace!.deliveries
    ...
    final monthlySpend = _workspace!.deliveries
        .where((item) {
  ...
  Widget _finance() {
    ...
    final methods = senderOrderedPaymentOptions(
      profile,
  ...
  Widget _accessRequestRow(BusinessAccessRequest request) => _OperationalRow(
    ...
    subtitle: '${request.email} · Requested ${_title(request.roleRequested)}',
      ...
      builder: (context) => SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _DetailLine(label: 'Outstanding', value: _gbp(invoice.balanceDue)),
            _DetailLine(label: 'Status', value: _title(invoice.status)),
            _DetailLine(label: 'Deliveries', value: '${invoice.deliveryCount}'),
              ...
              children: [
                Expanded(
                  child: _SecondaryButton(
                    label: 'Download PDF',
                    icon: Icons.picture_as_pdf_rounded,
                    onTap: () => _showMessage('Invoice PDF is being prepared.'),
                  ),
                ),
                ...
                Expanded(
                  child: _SecondaryButton(
                    label: 'Download CSV',
                    icon: Icons.table_view_rounded,
                    onTap: () => _showMessage('Invoice CSV is being prepared.'),
                  ),
                ),
              ],
            ),
  ...
  Future<void> _ensureCompanyCode({bool rotate = false}) async {
    ...
    setState(() => _working = true);
    try {
      final code = await _repository.ensureCompanyCode(
        account: account,
        rotate: rotate,
      );
      if (code.isEmpty) {
        ...
        onTap: widget.working
            ...
            : () => widget.onSave(
                BusinessAccount(
                  ...
                  businessAddressData: widget.account.businessAddressData,
                  ...
                  defaultPickupAddressData:
                      widget.account.defaultPickupAddressData,
  ...
  Widget build(BuildContext context) => Column(
    children: [
      Icon(
        ...
        size: 18,
        color: complete ? _success : _mutedDim,
      ),
...
class _MetricChartCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final maxValue = bars.values.fold<int>(
      0,
    ...
    child: Material(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          ...
          maxLines: 2,
          overflow: TextOverflow.ellipsis,
          style: GoogleFonts.jetBrainsMono(fontSize: 10.5, color: _mutedDim),
        ),
        trailing:
            trailing ??

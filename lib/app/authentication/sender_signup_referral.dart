...
String signupReferralFromRoute(String route) {
  try {
    if (RegExp(r'%(?![0-9a-fA-F]{2})').hasMatch(route)) return '';
    final uri = Uri.parse(route);
    final segments =
        (uri.fragment.startsWith('/') ? Uri.parse(uri.fragment) : uri)
            .pathSegments;
    return segments.length > 1 && segments.first.toLowerCase() == 'join'
        ? normalizeSignupReferral(segments[1])
        : '';
  } on FormatException {
    return '';
  }

...
class ProofOfDeliveryDetails {
  ...
  List<(String, String)> get visibleRows {
    final rows = <(String, String)>[
      ...
      if (riderName.trim().isNotEmpty) ('Rider', riderName),
    ...
    if (vanguard) {
      rows.insertAll(0, [
        ('Collection PIN', collectionPinVerified ? 'Verified' : 'Not verified'),
        ('Delivery PIN', deliveryPinVerified ? 'Verified' : 'Not verified'),
      ]);
    }
    return rows;
  }
...
ProofOfDeliveryDetails proofOfDeliveryFromRecord(
  ...
  final vanguard =
      _isTruthy(record['vanguardEnabled']) ||
  ...
  return ProofOfDeliveryDetails(
    ...
    deliveredAt: _formatDate(
      _firstValue(record, proof, const [
        'deliveredAt',
        'completedAt',
        'deliveryCompletedAt',
        'timestamp',
...
String _proofPhotoUrl(Map<String, dynamic> record, Map<String, dynamic> proof) {
  final proofScoped = _firstString(const <String, dynamic>{}, proof, const [
    'proofPhotoUrl',

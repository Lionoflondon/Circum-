...
function healthProjection(metric = {}) {
  ...
  const domains = {};
  for (const [domain, values] of Object.entries(metric.visualDomainMetrics || {})) {
    const domainRequests = Number(values.requestCount) || 0;
    const domainRatio = (value) => domainRequests ? Math.round(((Number(value) || 0) / domainRequests) * 10000) / 100 : 0;
    domains[domain] = {
      requestCount: domainRequests,
      successRatePercent: domainRatio(values.successCount),
      failureRatePercent: domainRatio(values.failureCount),
      reviewRatePercent: domainRatio(values.reviewCount),
      agreementRatePercent: domainRatio(values.agreementCount),
      disagreementRatePercent: domainRatio(values.disagreementCount),
    };
  }
  return {requestVolume: requests, latencyMs: {p50: percentileFromBuckets(metric.latencyBuckets, 0.5), p95: percentileFromBuckets(metric.latencyBuckets, 0.95), p99: percentileFromBuckets(metric.latencyBuckets, 0.99)}, failureRatePercent: ratio(metric.failureCount), timeoutRatePercent: ratio(metric.timeoutCount), unsupportedRatePercent: ratio(metric.unsupportedCount), lowConfidenceRatePercent: ratio(metric.lowConfidenceCount), fastPathRatePercent: ratio(metric.fastPathCount), reusedRequestRatePercent: ratio(metric.reusedRequestCount), riderDisagreementRatePercent: ratio(metric.riderDisagreementCount), adminOverrideRatePercent: ratio(metric.adminOverrideCount), adjudicatedTruthCount: Number(metric.adjudicatedTruthCount) || 0, classificationCorrectCount: Number(metric.classificationCorrectCount) || 0, weightBandCorrectCount: Number(metric.weightBandCorrectCount) || 0, classificationAccuracyPercent: adjudicatedRatio(metric.classificationCorrectCount, metric.classificationAdjudicatedCount), weightBandAccuracyPercent: adjudicatedRatio(metric.weightBandCorrectCount, metric.weightBandAdjudicatedCount), visualSystemStatus: metric.visualSystemStatus || "PRODUCTION", visualEvaluationMode: metric.visualEvaluationMode || "SHADOW", visualAuthorityMode: metric.visualAuthorityMode || "EVIDENCE_ONLY", visualShadowRequestCount: visualRequests, visualShadowSuccessRatePercent: visualRatio(metric.visualShadowSuccessCount), visualShadowFailureRatePercent: visualRatio(metric.visualShadowFailureCount), visualLowConfidenceRatePercent: visualRatio(metric.visualLowConfidenceCount), visualUnavailableRatePercent: visualRatio(metric.visualUnavailableCount), visualObjectAgreementRatePercent: visualRatio(metric.visualObjectAgreementCount), visualCategoryAgreementRatePercent: visualRatio(metric.visualCategoryAgreementCount), visualAgreementRatePercent: visualRatio(metric.visualAgreementCount), visualPartialAgreementRatePercent: visualRatio(metric.visualPartialAgreementCount), visualDisagreementRatePercent: visualRatio(metric.visualDisagreementCount), visualUnknownComparisonRatePercent: visualRatio(metric.visualUnknownComparisonCount), visualReviewSignalRatePercent: visualRatio(metric.visualReviewSignalCount), visualAdjudicatedTruthCount: Number(metric.visualAdjudicatedTruthCount) || 0, visualClassificationAccuracyPercent: adjudicatedRatio(metric.visualClassificationCorrectCount, metric.visualClassificationAdjudicatedCount), visualWeightBandAccuracyPercent: adjudicatedRatio(metric.visualWeightBandCorrectCount, metric.visualWeightBandAdjudicatedCount), visualHandlingAccuracyPercent: adjudicatedRatio(metric.visualHandlingCorrectCount, metric.visualHandlingAdjudicatedCount), visualRiskAccuracyPercent: adjudicatedRatio(metric.visualRiskCorrectCount, metric.visualRiskAdjudicatedCount), visualRiskFalsePositiveCount: Number(metric.visualRiskFalsePositiveCount) || 0, visualRiskFalseNegativeCount: Number(metric.visualRiskFalseNegativeCount) || 0, visualCorrectionCount: Number(metric.visualCorrectionCount) || 0, domainScorecards: domains};
}

...
function normalizeTraversalPoints(points) {
  ...
  if (normalized.length < 2) return {points: normalized, reason: "insufficient_server_observed_points"};
  let previousAt = null;
  for (let index = 0; index < normalized.length; index += 1) {
    const current = normalized[index];
    const currentAt = timestampMs(current.at);
    if (!Number.isFinite(currentAt)) return {points: [], reason: "invalid_movement_timestamp"};
    if (previousAt !== null) {
      const elapsedSeconds = (currentAt - previousAt) / 1000;
      if (elapsedSeconds <= 0) return {points: [], reason: "out_of_order_movement_timestamp"};
...
function evaluateActualTraversal({deliveryId, riderId, assignedVehicle, points} = {}) {
  ...
  if (normalized.length < 2 || normalizedResult.reason) {
    return {version: ACTUAL_TRAVERSAL_VERSION, deliveryId, riderId, status: "UNRESOLVED", evidenceCompleteness: "UNRESOLVED", reason: normalizedResult.reason};
  }
...
module.exports = {ACTUAL_TRAVERSAL_VERSION, MAX_POINTS, MAX_SEGMENT_SPEED_MPS, point, appendPoint, normalizeTraversalPoints, evaluateActualTraversal};

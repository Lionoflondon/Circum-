...
test("Rider withdrawal writes remain blocked from client-owned bank data", () => {
  assert.match(index, /exports\.createRiderTransferOrPayout = riderConnect\.createRiderTransferOrPayout/);
  assert.match(
      rules,
      /match \/payoutRequests\/\{requestId\}[\s\S]*allow create: if isAdmin\(\);/,
  );

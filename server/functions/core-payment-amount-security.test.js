...
test("canonical payment authority calculates and records authoritative pricing", () => {
  ...
  assert.match(senderBookingSource, /authoritativeDistanceMiles = authoritativeRoute\.distanceMetres \/ 1609\.344/);
  assert.match(senderBookingSource, /distanceMiles: authoritativeDistanceMiles/);
...
test("Sender Web checkout uses canonical payment callables and never writes paid deliveries directly", () => {
  ...
  assert.match(confirmPaymentSource, /circum-sender-delivery-payments-[^']+\/createSenderPaymentSession/);
  ...
  assert.match(confirmPaymentSource, /circum-sender-delivery-payments-[^']+\/createSenderPaidDelivery/);
  assert.match(webSenderSource, /circum-sender-delivery-payments-[^']+\/finalizeSenderWebCheckout/);

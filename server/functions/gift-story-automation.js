...
function safeStory(giftId, gift) {
  const voice = gift.voiceNote && typeof gift.voiceNote === "object" ? gift.voiceNote : {};
  const voiceDownloadUrl = text(voice.downloadUrl || voice.url || gift.voiceNoteUrl || gift.giftStorySenderVoiceNoteUrl);
  return {
    id: giftId,
    ...
    voiceNote: voiceDownloadUrl ? {
      hasVoiceNote: true,
      downloadUrl: voiceDownloadUrl,
      durationSeconds: Number(voice.durationSeconds || 0) || null,
      mimeType: text(voice.mimeType || ""),
      createdAt: voice.createdAt || voice.uploadedAt || null,
    } : null,
    giftStorySenderVoiceNoteUrl: voiceDownloadUrl || null,

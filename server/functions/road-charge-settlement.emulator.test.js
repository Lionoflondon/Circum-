...
const app = getApps().length ?
  getApps()[0] :
  initializeApp({projectId: "circum-road-charge-test"});
...
async function settle(deliveryId, charge, vehicleId = "vehicle-1") {
  ...
  const dailyRef = db
    .collection("testRoadDaily")
    .doc(dailyId(charge, vehicleId, charge.chargingDate));
  return db.runTransaction(async (transaction) => {
    ...
    transaction.set(
      dailyRef,
      {
        recoveredPence:
          (dailySnap.exists ?
            Number(dailySnap.data().recoveredPence || 0) :
            0) + effect.reimbursementPence,
...
emulatorTest(
  "emulator contention caps concurrent CCZ recovery at £18",
  async () => {
    ...
    const daily = await db
      .collection("testRoadDaily")
      .doc(dailyId(charge, "vehicle-1", day))
      .get();
    assert.equal(
      Math.min(1800, Number(daily.data().recoveredPence || 0)),
      1800,
    );
    assert.equal(
      results.reduce((sum, result) => sum + result.reimbursementPence, 0),
      1800,
    );
    assert.equal(
      new Set(
        results.map((result, index) =>
          result.duplicate ? `duplicate-${index}` : index,
        ),
      ).size,
      4,
    );
...
emulatorTest(
  "emulator retry creates no duplicate crossing effect",
  async () => {
    ...
    assert.equal(
      (await db.collection("testRoadEffects").doc(`crossing-${day}:ccz`).get())
        .exists,
      true,
    );
  },

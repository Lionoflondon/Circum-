...
async function markHealthPlusPaid({
  ...
  await paymentRef.set({
    ...
    productId: bookingId,
    healthPlusBookingId: bookingId,
    canonicalTransactionId: paymentId || payment.paymentId || bookingId,
    amountPence,
  ...
  try {
    ...
    if (!recurring && !split.stripeRequired) {
      await rothLedger.applyWalletDebit({
        ...
        metadata: {
          service: "health_plus",
          source: "health_plus_checkout",
          productType: "health_plus",
          productId: bookingId,
          healthPlusBookingId: bookingId,
          paymentId: bookingId,
          canonicalTransactionId: bookingId,
          totalAmount: orderTotalGbp,
  ...
  if (rothAmount > 0) {
    await rothLedger.applyWalletDebit({
      ...
      metadata: {
        ...
        paymentId: bookingId,
        canonicalTransactionId: bookingId,
        totalAmount: payment.amountPence == null ? null : Number(payment.amountPence) / 100,
        rothApplied: rothAmount,
        stripeAmount: cardAmount,

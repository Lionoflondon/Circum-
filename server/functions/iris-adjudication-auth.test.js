...
test("adjudicateIris requires admin authorization before any privileged access", () => {
  const functionStart = iris.indexOf("const adjudicateIris = functions");
...
test("analyseIris remains ordinary authenticated and is not admin-gated", () => {
  const analyseStart = iris.indexOf("const analyseIris = functions");
  const adjudicateStart = iris.indexOf("const adjudicateIris = functions");

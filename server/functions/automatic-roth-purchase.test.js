...
test("Roth purchase finalizers are idempotent", () => {
  assert.match(rothLedgerSource, /const \[existingLedger, wallet, senderWalletSnap, roleWalletSnap\]/);
  assert.match(rothLedgerSource, /if \(existingLedger\.exists\) \{[\s\S]*?sameMovement[\s\S]*?idempotent: true/);

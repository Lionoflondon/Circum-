...
const ROAD_CHARGE_POLICY = Object.freeze({
  ...
  charges: Object.freeze({
    congestion_charge: Object.freeze({
      ...
      chargingHours:
        "07:00-18:00 Monday-Friday; 12:00-18:00 weekends and bank holidays",
      settlementTreatment:
        "daily_vehicle_liability_half_customer_fee_recovery_waterfall",
...
function normalizeVehicle(value) {
  ...
  if (/(motorbike|motorcycle|motor bike|scooter|bike)/.test(normalized)) {
    return "motorbike";
  }
  return "unknown";
}
...
function vehicleTariffClassification(vehicleProfile = {}) {
  const vehicleClass = normalizeVehicle(
    vehicleProfile.type ||
      vehicleProfile.vehicleClass ||
...
function vanTunnelTariffAuthority(
  vehicleProfile = {},
  {pricingContext = "quote"} = {},
) {
  const verified = verifiedRoadChargeFacts(vehicleProfile);
  const declared = `${vehicleProfile.tunnelTariffClass || ""}`
    .trim()
    .toLowerCase();
  const referenceMassKg = Number(
    vehicleProfile.referenceMassKg || vehicleProfile.unladenWeightKg,
  );
  const explicit = /^(small_van|small van)$/.test(declared) ?
    "small_van" :
    /^(large_van|large van)$/.test(declared) ?
      "large_van" :
      null;
  const massClass =
    Number.isFinite(referenceMassKg) && referenceMassKg > 0 ?
      referenceMassKg < 1205 ?
        "small_van" :
        "large_van" :
      null;
  const conflict = explicit && massClass && explicit !== massClass;
  const actualClassification =
    verified && !conflict ? explicit || massClass : null;
  ...
  if (pricingContext === "quote") {
    return {
      ...
      pricingReason: conflict ?
        "CONFLICTING_VERIFIED_VEHICLE_DATA" :
        verified ?
...
function cczVehicleAuthority(
  ...
  const verified = verifiedRoadChargeFacts(vehicleProfile);
  const declared = `${vehicleProfile.cczAuthorityStatus || ""}`
    .trim()
    .toUpperCase();
...
function chargeIsEffective(charge, at) {
  ...
  const until = charge.effectiveUntil ?
...
function stableLiabilityKey({vehicleId, vehicleClass, date, chargeId}) {
  const owner = `${vehicleId || vehicleClass || "unknown_vehicle"}`
    .trim()
    .toLowerCase();
...
function liabilityRecord(liabilityState, key, amountPence) {
  ...
  const record =
    (liabilityState.liabilities && liabilityState.liabilities[key]) ||
    liabilityState[key];
  if (record && typeof record === "object") {
    return {
      incurred:
        record.incurred === true ||
        integerPence(record.riderRecoveryPence || record.riderReimbursedPence) >
          0,
      recoveredPence: Math.min(
        integerPence(amountPence),
        integerPence(
          record.recoveredPence ||
            record.riderRecoveryPence ||
            record.riderRecoveredPence ||
            record.riderReimbursedPence,
        ),
      ),
    };
  }
  if (
    record === true ||
    (Array.isArray(liabilityState.coveredKeys) &&
      liabilityState.coveredKeys.includes(key))
  ) {
...
function dailyRecoveryAllocation({
  liabilityPence,
  customerFeePence,
  recoveredPence,
}) {
  ...
  const riderRecovery = Math.min(
    customerFee,
    Math.max(0, liability - recoveredBefore),
  );
  return {
    ...
    remainingRecoveryPence: Math.max(
      0,
      liability - recoveredBefore - riderRecovery,
    ),
...
function evaluateRoadCharges({
  ...
  const charges = [];
  const facts =
    routeFacts && typeof routeFacts === "object" ? routeFacts : null;
  if (!facts || facts.authority !== "authoritative_route") {
    return summary({
      charges,
      routeKnown: false,
      reason: "authoritative_route_facts_unavailable",
    });
  }
  ...
  for (const crossing of crossings) {
    ...
    if (
      !charge ||
      charge.type !== "route_toll" ||
      !chargeIsEffective(charge, crossing.at || at)
    ) {
      continue;
    }
    ...
    const peak =
      chargeId === "blackwall_silvertown" ? tunnelPeak(crossing) : false;
    const active =
      chargeId === "blackwall_silvertown" ?
        tunnelChargeable(crossing) :
        chargeId === "dartford_crossing" ?
          dartfordChargeable(crossing) :
          true;
    const verifiedFacts = verifiedRoadChargeFacts(vehicleProfile);
    const vanTunnelAuthority =
      vehicleClass === "van" && chargeId === "blackwall_silvertown" ?
        vanTunnelTariffAuthority(vehicleProfile, {pricingContext}) :
        null;
    const conservativeDartford =
      vehicleClass === "van" &&
      chargeId === "dartford_crossing" &&
    ...
    const tunnelRateKey =
      vehicleClass === "van" && vanTunnelAuthority ?
        vanTunnelAuthority.pricingTariffApplied === "SMALL_VAN" ?
          "van_small" :
          "van_large" :
        vehicleClass;
    ...
    charges.push({
      ...
      ...(conservativeDartford ?
        {
            actualClassification: "UNKNOWN",
            pricingTariffApplied:
              pricingContext === "quote" ? "VAN_MULTI_AXLE_CONSERVATIVE" : null,
            pricingReason: verifiedFacts ?
              "INSUFFICIENT_VERIFIED_VEHICLE_DATA" :
              "UNVERIFIED_VEHICLE_DATA",
          } :
        {}),
  ...
  if (zone && zone.entered === true) {
    ...
    const active =
      chargeIsEffective(charge, zoneAt) &&
    ...
    const record = liabilityRecord(
      liabilityState,
      key,
      vehicleAuthority.liabilityPence || 0,
    );
    ...
    if (eligibleVehicle && active && !authorityKnownForSettlement) {
      ...
      !eligibleVehicle ||
      !active ||
      vehicleAuthority.status === "VERIFIED_EXEMPT"
    ) {
      charges.push({
        ...baseCharge({
          charge,
      ...
      charges.push({
        ...baseCharge({
          ...
          amountPence: vehicleAuthority.liabilityPence,
          liabilityKey: key,
          status: record.incurred ?
            "daily_liability_recovering" :
            "new_daily_liability",
        }),
        customerContributionPence: allocation.customerFeePence,
        customerContribution: moneyFromPence(allocation.customerFeePence),
    ...
    if (facts[complianceId] && facts[complianceId].applicable === true) {
      ...
      charges.push(
        baseCharge({
          ...
          amountPence: 0,
          status:
            facts[complianceId].compliant === true ?
              "compliant" :
...
function summary({charges, routeKnown, reason}) {
  const enriched = charges.map((charge) => {
    ...
    const customer =
      charge.customerContributionPence != null ?
        integerPence(charge.customerContributionPence) :
        charge.type === "route_toll" ?
          actual :
          0;
    const reimbursement =
      charge.riderReimbursementPence != null ?
        integerPence(charge.riderReimbursementPence) :
        charge.type === "route_toll" || charge.type === "daily_zone_charge" ?
          actual :
          0;
    return {
      ...charge,
  ...
  const sum = (field) =>
    enriched.reduce((total, charge) => total + integerPence(charge[field]), 0);
  return {
    policyVersion: ROAD_CHARGE_POLICY_VERSION,
    routeKnown,
    reason,
    ...
    liabilityKeys: enriched
      .filter((charge) => charge.liabilityKey)
      .map((charge) => charge.liabilityKey),
    authoritativePricingComplete:
      routeKnown &&
      !enriched.some((charge) =>
        [
          "unknown",
          "tariff_classification_unknown",
          "assigned_vehicle_identity_unknown",
...
function dispatchRoadChargeScore({routeFacts, rider, request, at} = {}) {
  const selectedVehicle =
    rider && (rider.vehicleType || rider.vehicleClass || rider.vehicle);
  const liabilityState = (rider && rider.roadChargeLiabilityState) || {};
  const result = evaluateRoadCharges({
    routeFacts,
    selectedVehicle:
      selectedVehicle ||
      (request && (request.vehicleType || request.requiredVehicle)),

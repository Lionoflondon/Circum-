...
const charge = {chargeId: "congestion_charge", type: "daily_zone_charge", customerContributionPence: 900};
const evidence = {authoritative: true, incurred: false};

test("unused scheduled road charge defaults to one Roth entitlement", () => {
  const entitlement = refunds.createEntitlement({deliveryId: "d1", quoteId: "q1", charge, actualEvidence: evidence});
...
test("unresolved actual incurrence never creates an automatic refund", () => {
  const entitlement = refunds.createEntitlement({deliveryId: "d2", quoteId: "q2", charge});
...
test("cash requires support authority and cannot double-settle Roth", () => {
  const pending = refunds.createEntitlement({deliveryId: "d3", quoteId: "q3", charge, actualEvidence: evidence});
  assert.equal(refunds.reserveCash(pending).decision.reason, "support_authorization_required");
  const roth = refunds.reserveRoth(pending, evidence);
  assert.equal(refunds.reserveCash(roth, {supportAuthorized: true}).decision.reason, "roth_reversal_required");
  const cash = refunds.settleCash(refunds.reserveCash(pending, {supportAuthorized: true}));
...
test("incurred liability produces no refund entitlement settlement", () => {
  const entitlement = refunds.createEntitlement({deliveryId: "d4", quoteId: "q4", charge, actualEvidence: {authoritative: true, incurred: true}});
  const result = refunds.reserveRoth(entitlement, {authoritative: true, incurred: true});
...
test("Roth settlement is deterministic and idempotent", async () => {
  ...
  const entitlement = refunds.createEntitlement({deliveryId: "d5", quoteId: "q5", charge, actualEvidence: evidence});
  entitlement.refundOwnerId = "sender-1";
  entitlement.refundablePence = 250;
  entitlement.entitlementPence = 400;
  store.set(`roadChargeRefundEntitlements/${entitlement.entitlementId}`, entitlement);
  const first = await refunds.settleEntitlementToRoth({db, entitlementId: entitlement.entitlementId, owner: {id: "sender-1"}});
  const second = await refunds.settleEntitlementToRoth({db, entitlementId: entitlement.entitlementId, owner: {id: "sender-1"}});

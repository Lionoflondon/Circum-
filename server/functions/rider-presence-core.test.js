...
test("goOnline never leaks raw internal failures to riders", () => {
  const source = fs.readFileSync(path.join(__dirname, "rider-presence.js"), "utf8");
  const goOnlineStart = source.indexOf("exports.goOnline = functions.runWith");
  const goOfflineStart = source.indexOf("exports.goOffline = functions.runWith");
...
test("heartbeat keeps riderProfiles dispatch state fresh", () => {
  const source = fs.readFileSync(path.join(__dirname, "rider-presence.js"), "utf8");
  const heartbeatStart = source.indexOf("exports.updateRiderPresence = functions.runWith");

...
const effectId = (deliveryId, charge, role) =>
  `${deliveryId}:${charge.chargeId || charge.key || "road_charge"}:${role}`.replace(
    /[^a-zA-Z0-9:_-]/g,
    "_",
  );

const dailyId = (charge, vehicleId, date) =>
  `${vehicleId || "unknown_vehicle"}:${date}:${charge.chargeId || "congestion_charge"}`.replace(
    /[^a-zA-Z0-9:_-]/g,
    "_",
  );

function planRoadChargeSettlement({
  deliveryId,
  riderId,
  delivery = {},
  assignedVehicle = {},
  ...
  return {
    ...
    dailyUpdates,
    reimbursementPence,
    reimbursement: money(reimbursementPence),
  };
}

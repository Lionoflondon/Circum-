...
function refundableAmountPence(entitlement = {}) {
  return pence(
    Object.prototype.hasOwnProperty.call(entitlement, "refundablePence") ?
      entitlement.refundablePence :
      entitlement.entitlementPence,
  );
}
...
function entitlementId({deliveryId, quoteId, chargeId, lineItemKey} = {}) {
  return [
    deliveryId,
    quoteId,
    chargeId || lineItemKey || "road_charge",
...
async function settleEntitlementToRoth({
  db = getFirestore(),
  entitlementId: id,
  owner = {},
} = {}) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (!entitlementSnapshot.exists) {
      return {settled: false, reason: "entitlement_not_found"};
    }
    ...
    if (
      !entitlement.deliveryId ||
      !entitlement.quoteId ||
      !entitlement.chargeId
    ) {
      return {
        settled: false,
        reason: "malformed_entitlement",
        entitlementId: id,
      };
    }
    const amountPence =
      refundableAmountPence(entitlement) - pence(entitlement.cashRefundedPence);
    if (
      entitlement.state === STATES.rothSettled ||
      entitlement.state === STATES.closed
    ) {
    ...
    if (entitlement.state !== STATES.eligible || amountPence <= 0) {
      return {
        settled: false,
        reason:
          entitlement.state === STATES.pending ?
            "evidence_unresolved" :
            "not_eligible",
        entitlementId: id,
      };
    ...
    if (ownerType === "business") {
      ...
      const previous = Number(
        (walletSnapshot.data() && walletSnapshot.data().balance) || 0,
      );
          ...
          availableBalance:
            Math.round(
              Math.max(
                0,
                resulting -
                  Number((walletSnapshot.data() || {}).reservedBalance || 0),
              ) * 100,
            ) / 100,
      ...
      transaction.create(txRef, {
        ...
        amountGBP: amount,
        currency: "GBP",
        type: "scheduled_road_charge_refund",
        note: "Unused scheduled road charge returned to Business Roth.",
        relatedEntityId: entitlement.deliveryId,
        entitlementId: id,
      ...
      const walletId =
        entitlement.refundOwnerWalletId || walletIdForEmail(email) || uid;
      if (!walletId || !uid) {
        return {
          settled: false,
          reason: "sender_owner_missing",
          entitlementId: id,
        };
      }
      ...
      const previous = Number(
        (walletSnapshot.data() &&
          (walletSnapshot.data().balance ??
            walletSnapshot.data().rothCredit)) ||
          0,
      );
      const resulting = previous + amount;
      transaction.set(
      ...
      transaction.set(
        ...
        senderWalletProjectionRecord({
          ...
          version:
            Number(
              (senderSnapshot.data() && senderSnapshot.data().version) || 0,
            ) + 1,
          createdAt:
            (senderSnapshot.data() && senderSnapshot.data().createdAt) ||
      ...
      transaction.create(txRef, {
        ...
        type: "scheduled_road_charge_refund",
        reason: "Unused scheduled road charge returned to Roth.",
        relatedEntityId: entitlement.deliveryId,
        entitlementId: id,
        status: "completed",
        createdAt: FieldValue.serverTimestamp(),
    ...
    transaction.update(entitlementRef, {
      state: STATES.rothSettled,
      refundedPence: amountPence,
      rothCreditedPence: amountPence,
      settlementReference: `road_charge_refund_${id}`,
...
async function settleEntitlementToCash({
  ...
  if (actor.authorized !== true || !actor.uid) {
    return {
      settled: false,
      reason: "support_authorization_required",
      entitlementId: id,
    };
  }
  const requestReference = cleanReference(
    customerRequestReference,
    "Customer support request reference",
  );
  const refundReference = cleanReference(
    cashRefundReference,
    "Cash refund reference",
  );
  const entitlementRef = db.collection("roadChargeRefundEntitlements").doc(id);
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (!supportRequestSnapshot.exists) {
      return {
        settled: false,
        reason: "support_request_not_found",
        entitlementId: id,
      };
    }
    const entitlement = entitlementSnapshot.data() || {};
    ...
    if (
      supportRequest.deliveryId &&
      supportRequest.deliveryId !== entitlement.deliveryId
    ) {
      return {
        settled: false,
        reason: "support_request_delivery_mismatch",
        entitlementId: id,
      };
    }
    if (entitlement.policyVersion !== REFUND_POLICY_VERSION) {
    ...
    const amountPence =
      refundableAmountPence(entitlement) -
      pence(entitlement.rothCreditedPence) -
    ...
    if (
      entitlement.state === STATES.rothSettled ||
      pence(entitlement.rothCreditedPence) > 0
    ) {
      return {
        settled: false,
        reason: "roth_already_settled",
        entitlementId: id,
      };
    }
    if (entitlement.state !== STATES.eligible || amountPence <= 0) {
      return {
        settled: false,
        reason: "cash_refund_not_eligible",
        entitlementId: id,
      };
    }
...
const settleScheduledRoadChargeCashRefund = functions
  .runWith({enforceAppCheck: true})
  .https.onCall(async (data, context) => {
    const actorUid = requireAdmin(
      context,
      "Support or administrator access is required.",
    );
...
function eligibleRefund(entitlement, actualEvidence) {
  ...
  if (
    !entitlement ||
    (entitlement.state !== STATES.pending &&
      entitlement.state !== STATES.eligible)
  ) {
  ...
  return amountPence > 0 ?
    {eligible: true, amountPence} :
    {eligible: false, reason: "already_settled"};
}
...
function reserveRoth(entitlement, actualEvidence) {
  ...
  return {
    ...entitlement,
    state: STATES.rothSettled,
    rothCreditedPence: decision.amountPence,
    refundedPence: decision.amountPence,
    decision,
  };
}

function reserveCash(entitlement, actor = {}) {
  if (!actor.supportAuthorized) {
    return {
      ...entitlement,
      decision: {eligible: false, reason: "support_authorization_required"},
    };
  }
  if (!entitlement || entitlement.policyVersion !== REFUND_POLICY_VERSION) {
    return {
      ...entitlement,
      decision: {eligible: false, reason: "unsupported_policy_version"},
    };
  }
  if (pence(entitlement.rothCreditedPence) > 0) {
    return {
      ...entitlement,
      state: STATES.review,
      decision: {eligible: false, reason: "roth_reversal_required"},
    };
  }
  if (entitlement.state !== STATES.eligible) {
    return {
      ...entitlement,
      decision: {eligible: false, reason: "entitlement_not_cash_reservable"},
    };
  }
  ...
  if (amountPence <= 0) {
    return {
      ...entitlement,
      decision: {eligible: false, reason: "cash_refund_not_eligible"},
    };
  }
  return {
    ...entitlement,
    state: STATES.cashReserved,
    decision: {eligible: true, amountPence},
  };
}

function settleCash(entitlement) {
  if (!entitlement || entitlement.state !== STATES.cashReserved) {
    return {
      ...entitlement,
      decision: {eligible: false, reason: "cash_not_reserved"},
    };
  }
  const amountPence = refundableAmountPence(entitlement);
  return {
...
function invariant(entitlement) {
  const total =
    pence(entitlement && entitlement.rothCreditedPence) +
    pence(entitlement && entitlement.cashRefundedPence);
...
module.exports = {
  ...
  settleScheduledRoadChargeCashRefund,
  invariant,
};

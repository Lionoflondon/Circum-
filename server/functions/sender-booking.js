...
exports.createSenderPaymentSession = (stripe) => functions.runWith({enforceAppCheck: true}).https.onCall(async (data, context) => {
  ...
  if (webCheckout) {
    ...
    await sessionRef.set({
      ...
      futureUsageAuthorized: true,
      futureUsageAuthority: "stripe_checkout_setup_future_usage_off_session",
      requestId,
      draftId: draftId || null,
      idempotencyKey,
      deliveryPayload,
  ...
  await sessionRef.set({
    ...
    stripeCustomerId: customerId,
    futureUsageAuthorized: true,
    futureUsageAuthority: savedPaymentMethodId ? "saved_payment_method_setup_intent" : "payment_intent_setup_future_usage_off_session",

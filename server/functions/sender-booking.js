...
exports.saveSenderDraft = functions.runWith({enforceAppCheck: true}).https.onCall(async (data, context) => {
...
exports.loadSenderDraft = functions.runWith({enforceAppCheck: true}).https.onCall(async (_data, context) => {
  const sender = requireSender(context);
...
exports.deleteSenderDraft = functions.runWith({enforceAppCheck: true}).https.onCall(async (_data, context) => {
  const sender = requireSender(context);
  ...
  return {
    ...
    ...(serverPhotoAnalysis ? {
      ...
      photoAnalysis: {
        ...
        handlingFlags: Array.isArray(serverPhotoAnalysis.handlingFlags) ? serverPhotoAnalysis.handlingFlags : [],
        recommendedVehicle: serverPhotoAnalysis.recommendedVehicle || null,
        vanguardRequired: serverPhotoAnalysis.vanguardRequired === true,
        vanguardRecommended: serverPhotoAnalysis.vanguardRecommended === true,
        complianceStatus: serverPhotoAnalysis.complianceStatus || null,
        complianceReasonCodes: Array.isArray(serverPhotoAnalysis.complianceReasonCodes) ? serverPhotoAnalysis.complianceReasonCodes : [],
        serviceabilityStatus: serverPhotoAnalysis.serviceabilityStatus || null,
...
exports.getSenderRothBalance = functions.runWith({enforceAppCheck: true}).https.onCall(async (_, context) => {
  const sender = requireSender(context);
...
exports.createSenderBookingQuote = functions.runWith({enforceAppCheck: true, secrets: ["GOOGLE_ROUTES_API_KEY"]}).https.onCall(async (data, context) => {
  const sender = requireSender(context);
  const db = getFirestore();

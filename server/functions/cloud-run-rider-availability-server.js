...
const {processAvailabilityEvent} = require("./rider-availability-worker-core");
...
function createServer(options = {}) {
  ...
  return http.createServer((request, response) => {
    ...
    request.on("end", async () => {
      ...
      try {
        const event = parseAvailabilityEvent({headers: request.headers, body: Buffer.concat(chunks), collection});
        if (!event.relevant) {
          console.info(JSON.stringify({event: "rider_availability_filtered", eventId: event.eventId, riderId: event.riderId, sourceCollection: collection, outcome: "IGNORED"}));
          return json(response, 200, {outcome: "IGNORED", riderId: event.riderId});
        }
...
module.exports = {createServer, EVENT_PATHS, MAX_EVENT_BODY_BYTES, productionProcessor};

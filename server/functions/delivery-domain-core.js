...
const CORE_FIELDS = Object.freeze([
  "deliveryId", "status", "senderId", "recipientId", "riderId",
  "createdAt", "completedAt", "dispatchId", "trackingId", "pricingId",
  "paymentId", "evidenceId", "eventVersion",
]);

const CANONICAL_STATUSES = Object.freeze([
  "requested", "accepted", "navigating_to_pickup", "arrived_at_pickup",
  "pickup_verified", "collected", "navigating_to_dropoff",
  "arrived_at_dropoff", "delivered", "cancelled", "failed",
]);
...
function createDeliveryCore(input = {}) {
  ...
  return Object.freeze({
    deliveryId,
    status: canonicalStatus(input.status || input.deliveryStatus || "requested"),
    senderId: optionalId(input.senderId || input.userId || input.customerId),
    recipientId: optionalId(input.recipientId || input.receiverId || input.recipientUserId),
    riderId: optionalId(input.riderId || input.assignedRiderId || input.driverId),
...
function assertDeliveryCore(core) {
  ...
  for (const key of Object.keys(candidate)) {
    if (!CORE_FIELDS.includes(key)) throw new TypeError(`Unknown core field: ${key}`);
  }
  return candidate;
}
...
function extensionReference({context, id, version = 1} = {}) {
  ...
  return Object.freeze({context: name, id: extensionId, version: Number(version) || 1});
}

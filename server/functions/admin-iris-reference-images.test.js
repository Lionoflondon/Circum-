...
const adminPermissions = fs.readFileSync(
    path.join(__dirname, "admin-permissions.js"),
    "utf8",
);
...
test("IRIS reference image workflow is admin-only and audited", () => {
  ...
  assert.match(adminPermissions, /super_admin/);
  assert.match(adminPermissions, /operations_admin/);
  assert.match(source, /adminCallable/);

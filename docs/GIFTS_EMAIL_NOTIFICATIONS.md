...
Delivery confirmation is created in the canonical Firestore `emailQueue` with a deterministic Gift/event identity. The queue is private, prevents duplicate delivery emails, and retries transient provider failures. It never includes the delivery address or private Gift Story content in the email.
...
4. Deploy only the changed Gifts publisher and the private
